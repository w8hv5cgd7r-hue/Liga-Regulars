...
export function RoundForm({
  ...
  const strokeFront =
    (modality === "stroke" || isMatch) && frontHoles.length
      ? computeStrokePlay(frontHoles, playerScoresForEngine)
      : [];
  const strokeBack =
    (modality === "stroke" || isMatch) && backHoles.length
      ? computeStrokePlay(backHoles, playerScoresForEngine)
      : [];
      ...
      {showCard && holes.length > 0 && selectedPlayers.length > 0 && (
        <section className="rounded-lg border border-border bg-card p-4">
          ...
          <p className="mt-2 text-xs text-muted">
            ...
            ya han metido su golpe todos los jugadores de esa tarjeta. Toca la casilla de golpes para
            escribir el número con el teclado del móvil.
          </p>
              ...
              {matchPreview ? (
                <div className="flex flex-col gap-2">
                  ...
                  <div className="border-t border-border pt-2">
                      ...
                      <table className="w-full text-xs text-muted">
                        ...
                        <tbody>
                          {selectedPlayers.map((p) => {
                            ...
                            return (
                              <tr key={p.id}>
                                <td className={`py-1 pr-2 font-medium ${sideTextClass(p.id, teamA, teamB)}`}>
                                  {p.full_name.split(" ")[0]}
                                </td>
                                {showBackNine && (
                                  <td className="px-1 py-1 text-center">
                                    {fmt(strokeFront.find((r) => r.player_id === p.id))}
                                  </td>
                                )}
                                {showBackNine && (
                                  <td className="px-1 py-1 text-center">
                                    {fmt(strokeBack.find((r) => r.player_id === p.id))}
                                  </td>
                                )}
                                <td className="px-1 py-1 text-center">
                                  {fmt(strokeTotal.find((r) => r.player_id === p.id))}
                                </td>
                              </tr>
                            );
                          })}
                    ...
                    {useHandicap && (
                      <p className="mt-1 text-[11px] text-muted">Neto (bruto entre paréntesis).</p>
                    )}
                  </div>
                </div>
              ) : (
...
function StrokeStepper({
  ...
  // El campo lleva su propio "borrador" de texto en vez de estar atado
  // directamente al número guardado. Así, si el usuario borra el dígito
  // para corregirlo, el campo se queda vacío en pantalla en vez de
  // recuperar el número anterior en cuanto algo más en la página se
  // vuelve a renderizar (por ejemplo, el autoguardado de otro hoyo).
  // Solo se avisa al padre (y se guarda) cuando hay un número válido.
  const [draft, setDraft] = useState(value != null ? String(value) : "");
  const focused = useRef(false);

  // Si el valor viene de fuera (por ejemplo, otro jugador lo actualiza en
  // vivo) y el usuario no está escribiendo ahora mismo en este campo, se
  ...
  return (
    <input
      ...
      value={draft}
      placeholder="–"
      aria-label="Golpes de este hoyo (toca para escribir el número con el teclado)"
      onFocus={(e) => {
      ...
      onBlur={() => {
        ...
        // Si se queda vacío al salir del campo (por ejemplo, lo borró
        // para escribir otro número pero cambió de idea), se recupera el
        // último valor guardado en vez de dejarlo en blanco.
        setDraft(value != null ? String(value) : "");
      }}
      className="h-9 w-10 rounded-md border border-border bg-transparent text-center text-base tabular-nums focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
    />
  );
}
